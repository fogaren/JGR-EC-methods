...
% Look at imaginary part to make sure it's zero
    ...
    disp('ERROR...the imaginary part of eta timeseries is too big')
    end

    imag_test = imag(u(9));
    if abs(imag_test) > 1e-6
    disp('ERROR...the imaginary part of u timeseries is too big')
    end

    imag_test = imag(w(9));
    if abs(imag_test) > 1e-6
    disp('ERROR...the imaginary part of w timeseries is too big')
    end

Hsig = 4.*sqrt(sum(abs(Feta/(npoints)).^2));

[~,fin] = max(Feta(1:lfreq));
fp = freq(fin);
Tp = 1./fp;                  